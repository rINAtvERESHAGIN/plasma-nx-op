...
export class CoreDataActionCreatorService {
  static setLabs(data: Lab[]): ThunkAction<void, RootState, unknown, any> {
    return (dispatch) => {
      dispatch(setData({ key: '_labs', data }));
    };
  }

  static setParameters(data: Parameter[]): ThunkAction<void, RootState, unknown, any> {
    return (dispatch) => {
      dispatch(setData({ key: '_parameters', data }));
    };
  }

  static setRegions(data: AllRegion): ThunkAction<void, RootState, unknown, any> {
    return (dispatch) => {
      dispatch(setData({ key: '_regions', data }));
    };
  }

  static setTimeline(data: Timeline[]): ThunkAction<void, RootState, unknown, any> {
    return (dispatch) => {
      dispatch(setData({ key: '_timeline', data }));
    };
  }

  static setProcessors(data: any): ThunkAction<void, RootState, unknown, any> {
    return (dispatch) => {
