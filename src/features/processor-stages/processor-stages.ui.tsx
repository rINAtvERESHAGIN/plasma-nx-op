...
const ProcessorStages: React.FunctionComponent = ({ initialData, updateBlock }: IProps): any => {
    ...
    if (
      ...
      processingResult(dataset, processor)
        .then(async ({ dataset, processor }) => {
          if (!isNil(uuidResult)) await restoreResultFromDB(uuidResult);
          else await loadResultFromServer({ dataset, processor });
        })
        .finally(() => {
...
export default React.memo(ProcessorStages, arePropsEqual);
