...
const StatsBirthsDeathsPlotly = (): React.ReactNode => {
  const chartId: ChartId = ChartIds.STATS_BIRTHS_DEATHS_PLOTLY;
    ...
    const getChart = async (): Promise<void> => {
      try {
        const response = await __request(OpenAPI, {
          method: 'POST',
          url: '/api/births_deaths/',
          body: { id: regions.data[region.data.iso_code].id }
        });
...
export default StatsBirthsDeathsPlotly;
